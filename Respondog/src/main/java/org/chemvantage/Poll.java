...
public class Poll extends HttpServlet {
	...
	static String instructorPage(User user,Assignment a,HttpServletRequest request) throws Exception {
		...
		if (a.pollIsClosed) { 
			buf.append("<form method=post action=/Poll target=_respondog onsubmit=document.getElementById('sbmt3').disabled=true; >"
				+ "<input type=hidden name=sig value='" + user.getTokenSignature() + "' />"
	...
	private static String waitForPoll(User user,Assignment a) throws Exception {
		...
		buf.append("<form method=get action=/Poll target=_respondog onsubmit=document.getElementById('sbmt2').disabled=true; />"
				+ "<input type=hidden name=sig value='" + user.getTokenSignature() + "' />"
